<x-action-section>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-background-900 dark:text-background-100">
                        ...
                        @if (! is_null($account))
                            <div class="flex items-center space-x-6">
                                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos() && ! is_null($account->avatar_path))
                                    <button class="cursor-pointer ml-6 text-sm text-background-500 dark:text-background-400 hover:text-background-700 dark:hover:text-background-300 focus:outline-none" wire:click="setAvatarAsProfilePhoto({{ $account->id }})">
